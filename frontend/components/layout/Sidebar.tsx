...
function ProjectSelectItem({ listProjects }: { listProjects: projectType[] }) {
  ...
  return (
    <Disclosure as="div" className="ml-3 pb-2">
      {({ open }) => (
        <>
          <Disclosure.Button
            className=" border-4Background hover:bg-gradient-to-r hover:border-main border w-full text-center text-muted hover:text-mainText
          flex items-center px-3 py-2 rounded font-medium text-sm relative overflow-hidden duration-200"
          >
            <span className="text-center text-base text-secondaryText w-5/6 overflow-hidden overflow-ellipsis">
              {currentProject}
            </span>
            <svg
              className={classNames(
                open ? 'text-secondaryText rotate-90' : 'text-gray-300',
                'ml-3 h-5 w-5 transform transition-colors duration-200 ease-in-out group-hover:text-gray-400',
              )}
              viewBox="0 0 20 20"
              aria-hidden="true"
            >
          ...
          <Disclosure.Panel className="space-y-1">
            {listProjects.map((project) => (
              <Link key={project.id} href={`/project/${project.id}`}>
                <Disclosure.Button
                  as="a"
                  className="my-1 group cursor-pointer flex w-full items-center rounded-md py-2 pl-11 pr-2 text-sm font-medium text-muted hover:bg-secondaryBackground hover:text-mainText duration-200"
                >
                  {project.name}
                </Disclosure.Button>
              </Link>
