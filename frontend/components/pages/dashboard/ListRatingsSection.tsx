...
export const ListRatingsSection = ({ ratings }: { ratings: ratingType[] }) => {
  ...
  const sortRatingsAscending = (e: MouseEvent<HTMLButtonElement>) => {
    ...
    let newratings = [];
    if (category === 'avg') {
      newratings = [...ratings].sort(
        (a, b) =>
          Math.round((a.design + a.speed + a.responsive) / 3) - Math.round((b.design + b.speed + b.responsive) / 3),
      );
  ...
  const sortRatingsDescending = (e: MouseEvent<HTMLButtonElement>) => {
    ...
    if (category === 'avg') {
      newratings = [...ratings].sort(
        (a, b) =>
          Math.round((b.design + b.speed + b.responsive) / 3) - Math.round((a.design + a.speed + a.responsive) / 3),
      );
    } else if (category === 'createdAt') {
