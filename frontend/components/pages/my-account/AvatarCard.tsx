...
export const AvatarCard = ({ profile }: { profile: userType }) => {
  ...
  return (
    <div className="py-8 self-center space-y-4">
      <img className="inline-block h-32 w-32 rounded-full" src={session?.user.image} alt="user avatar" />
      {profile.provider === 'github' ? (
        ...
        <div className="text-sm font-medium hover:text-main text-center duration-200">{profile.username}</div>
      )}
    </div>
  );
};
